class App {
  ...
  count() {
    const weightDifference = this.getWeighDifference();
  ...
  getTemplate(
    ...
    <section class="result" data-result>
      <p class="result__paragraph">You want ${text} <strong>${this.getValueWithUnit(
      ...
      ${
        changePerWeek
          ? `<p class="result__paragraph">
      You should loose <strong>${this.getValueWithUnit(
    ...
    `
      : `
    <section class="result" data-result>
      <p class="result__paragraph"> Current and desired weight can't be equal, start and end date should be different
    </p>`;
  }
  ...
  getChangePerDay() {
    ...
    const changePerDay = numberOfDays
      ? this.getWeighDifference() / numberOfDays
      : null;
  ...
  getChangePerWeek() {
    ...
    const changePerWeek = numberOfWeeks
      ? this.getWeighDifference() / numberOfWeeks
      : null;
